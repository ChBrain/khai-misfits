---
khai: plot
title: "Everything In There Gets the Dose"
language: english
license: CC-BY-NC-SA-4.0
stamp:
  owner: KAI HACKS AI
  version: v0.0.1
  date: "2026-07-31"
---

# Plot: Everything In There Gets the Dose

## Taxonomy

[Never Aimed at You](play_never_aimed_at_you.md)

## Owner

- Project: khai-misfits

## Cue

[The Flora](place_the_flora.md), where a systemic drug enters a body containing a great deal more than the infection.

## Action

Through [Exposing](process_exposing.md), [the Patient](persona_the_patient.md) on [Finish the Course](plan_finish_the_course.md) sets [the Organism Nobody Is Treating](piece_the_organism_nobody_is_treating.md): the pneumococcus in the throat, the staphylococcus on the skin, the enterobacteria in the gut, none of them causing anything, all of them selected on, and the survivors staying. The course is completed exactly as instructed. The scene closes with the community not returning to what it was, and with nobody in the encounter having felt, seen or recorded any of it (Dethlefsen and Relman).

## Stage

The patient did what they were told, and a patient who stopped early would be criticised for it by the same system. There is no version of complying more carefully that changes where a systemic drug goes.

## Tension

The event that matters produces no evidence at the moment it happens, so the one participant with no decision to make is the one the whole mechanism actually runs through.
