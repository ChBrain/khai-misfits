---
khai: position
title: "The Repertoire"
language: english
license: CC-BY-NC-SA-4.0
stamp:
  owner: KAI HACKS AI
  version: v0.0.1
  date: "2026-07-21"
---

# Position: The Repertoire

## Taxonomy

[The Good Move](play_the_good_move.md). The practised seat, a stock of methods that solved their kind of problem before and fire again at once, the store that is both the expertise and the block.

## Owner

- Project: khai-misfits

## Has

A stock of solutions, ready and fast.
The seat holds the methods that repeated success has installed, so a problem of a known kind is answered almost before it is read, the whole store standing ready to fire on a match.
Depth as its whole asset: the seat's value is exactly how much it holds and how fast it delivers, since a wide, quick repertoire is what lets it solve at a glance what an empty one must labour at.

## Orders

Meet the problem with the method that fits.
Match the new case to the kind it resembles, apply the solution that kind has always yielded, and take the answer it produces, since the method is sound and the match is real and the answer works.
Trust the store, since the seat's whole method is to reach into what it has and bring out what fits, and the store has earned that trust across every problem it has already solved.

## Loses

The clear field a better move would need, which the seat crowds by its own richness.
The Repertoire cannot both hold a deep, fast store and keep the board open, because the same readiness that delivers the good answer first is what spends the attention before a search for better can begin, so the seat's asset is its liability in one motion: the fuller the store, the faster it fires, and the faster it fires, the sooner the looking closes on what it produced. What the seat forfeits is the even search across the whole problem, given up not by neglect but by design, since a store worth having is a store that answers before you ask, and an answer that arrives before the question is one that fills the space a better answer would have occupied, so the seat that makes the solver expert is the seat that makes them blind on exactly the problems where a better move was there to find.

## Drives

Toward a good answer delivered fast and a better one crowded out,
and a field spoken for before it is searched,
so the seat that holds the expertise is the same seat whose richness closes the looking, its depth the reason the better move is never reached.
