...
The research warrant for [The High-Water Mark](play_the_high_water_mark.md).
The dynamic is the fiscal ratchet, the displacement effect: Peacock and Wiseman's finding that public spending rises to
meet a crisis and does not return, Wagner's secular upward tendency behind it, Olson's distributional coalitions that
entrench each commitment, Buchanan and Wagner's asymmetry between granting and withdrawing, and the flypaper stickiness
of a placed outlay, all cited below.

## Line of Work

Staging the fiscal ratchet as a **commitment-floor displacement structure**: a crisis raises the spending to meet a real
and present need, and when the crisis passes the spending does not fall back, because the outlay has bedded into the
baseline and gathered a constituency, so the level settles above the old floor at a new plateau and the next crisis
climbs from that mark, not the old line. The play models the documented mechanics, the crest a crisis licenses, the
...
performance target and the engine is strategic underperformance to keep the bar low, a party deliberately hiding what it
can do. Here nothing is a target and no one soldiers: the ratcheted thing is a spending floor lifted by a real crisis
and held by real beneficiaries, and the asymmetry is structural, a commitment easy to add because its cost is diffuse
and near-impossible to remove once a constituency depends on it, not a metric gamed but a floor that only climbs. It is
distinct too from The New Normal, where a shifting baseline recalibrates what registers as normal, a change in
...
- **No villains**: the elements are reduced to load-bearing vectors. The play refuses to cast the spenders as profligate
  or the beneficiaries as takers, because the source finding is precisely that no waste and no greed is needed: each
  outlay answers a real and present need, correct at the moment of granting, and the crisis that licenses it is genuine
  (Peacock and Wiseman). The beneficiaries who defend the commitment are not parasites but people who came to depend on
  a floor they were invited to trust, and their defence is honest and proportionate (Olson). The asymmetry that makes
  granting easy and cutting dear is not a scheme but a structure, a diffuse cost no one will fight against a concentrated
  benefit someone will (Buchanan and Wagner), so the misfit is that a level raised to meet a true crisis outlives it and
  climbs, no profligacy and no plan required. The rising floor is the sum of sound commitments each of which met a real
  need, not the product of anyone's excess, and the hand that funds the emergency is as reasonable as the constituency
  that keeps what it was given.
- **No hero**: the play refuses to resolve the misfit into a state trimmed back by resolve alone. The one move that would
  lower the floor is to sunset the commitment when its crisis ends and enforce the drawdown against the constituency, but
  that means inflicting a concentrated, immediate, nameable loss on identifiable people for a diffuse, deferred, unowned
...
- **[The Waterline](piece_the_waterline.md)**: carries the concentrated-benefit asymmetry (Olson), the inhabited level a
  ...
  commitment.
- **[The Sunset](piece_the_sunset.md)**: carries the remedy that fails on contact with the constituency (Buchanan and
  Wagner), the expiry easy to write when no one depends on the outlay and impossible to enforce once they do, the
  drawdown that lapses exactly when it is needed.
