...
Staging the cobweb cycle as a **lagged-supply-answers-current-price structure**: production takes time, so a producer
must commit output before it exists and plans on the price in front of them, but what is committed now is delivered
...
disturbance originates in demand and grows by propagation along a chain of several firms, while here demand is steady,
the disturbance originates in a single market's own supply response to price, and the oscillation is endogenous to one
production lag rather than amplified across links, a market whipsawing itself in time rather than a signal magnifying
up a chain.
...
| **The veterinary workforce record** | the profession's own supply and demand analyses of the 1980s, and the practitioner incomes that followed | **The cobweb with a forecast attached, in a professional labour market**: training capacity expanded against a demand for services growing far more slowly, and the profession's analysts projected the overshoot roughly fifteen years ahead, putting supply growth at about three quarters against demand growth of a quarter to a third across 1980 to 2000 and predicting real practitioner incomes falling to around three quarters of the 1980 level. The interest of the case is that the forecast was published, was broadly borne out, and did not change the plan, which is the swing arriving in a market that had been told: the lag here is a training pipeline rather than a breeding season, and the entrants commit years before the price they will meet exists. Figures carried at the strength of the profession's later retrospective of those analyses rather than of the primary studies. The Swing, The Delay. |

## Restrictions

What the house refuses to model, and how it credits what it stages.

- **No villains**: the elements are reduced to load-bearing vectors. The play refuses to cast the producers as fools or
  the swing as a panic, because the source finding is precisely that no miscalculation and no shock is needed: each
  producer reads a true price and commits a sound plan, the correct response to the market at the moment of planting,
  and the lag is an honest fact of making things that take time. No single plan is wrong on the price it saw, and the
  overshoot is created by every producer answering the same signal at once, so the misfit is that a true price read on
  a lag and answered in chorus overshoots itself, no error and no outside disturbance required. The boom and bust are
  the sum of correct individual choices, not the product of anyone's greed or blunder, and the grower who plants big
  into a high price is as sensible as the buyer who pays the going rate.
- **No hero**: the play refuses to resolve the misfit into a market steadied by better sense alone. The one move that
  would still the swing is to plant on the expected price rather than the current one, to hold output level against the
  boom and the slump, but that means distrusting the plainest signal the market offers and betting on a forecast, and,
  while others keep chasing the price, exposes the steady producer to their mistiming and to a run of years when the
  crowd's choice looked right and theirs looked foolish. The reckoning is a producer who forgoes the visible price for
  a predicted one, or a market that swings because everyone, sensibly, trusts the price they can see, not a way to plan
  on the current signal and escape the cycle it drives, so the cure for a self-feeding swing is a deliberate distrust of
  the truest number the market shows.
- **Credit, not claim**: the research above is other people's work, packaged and attributed here, never claimed as the
  house's, and its reach is not overstated. The staging is the house's; the cobweb theorem and its convergence cases
  are Ezekiel's, the determinateness classification is Kaldor's, the empirical hog cycle is Hanau's, the dependence of
  the cycle on how expectations are formed is Nerlove's, and the rational-expectations correction is Muth's, each
  carried as carefully as the finding itself: the misfit stages a true price answered on a lag and credits that no
  producer miscalculated and the swing is the market's own.
