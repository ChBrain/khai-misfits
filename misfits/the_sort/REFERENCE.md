...
Staging Schelling's tipping as a **mild-preference-tips-into-stark-sort structure**: each resident holds a modest,
tolerant preference about the mix of their local neighbours, wanting only not to be too small a minority, and will
move to satisfy it, so no one prefers segregation and a mixed street meets everyone's wish at once, yet the wish
carries a threshold, and one move lowers a block's like-share enough to tip a neighbour past their line, whose move
tips the next, until the block empties of one kind and fills with the other and the city sorts itself. The play
models the documented mechanics, the mild preference that is the whole of what anyone wants, the threshold hidden
...
| **Anne E. Lincoln**     | "The Shifting Supply of Men and Women to Occupations: Feminization in Veterinary Education" (_Social Forces_, 2010)           | **The sort off a residential board and into a career**: the veterinary applicant pool of 1975 to 1995 read as the same tipping dynamic working on occupational choice rather than on a neighbourhood, where applicants respond to the composition they observe and the response reshapes it, the mechanism shown to run on a substrate with no streets in it. Carried with the paper's own three factors, the 1972 federal amendment ending discrimination against female students, the deterrence of applicants by an observed composition, and the rising supply of women earning bachelor's degrees, of which only the second is this misfit's mechanism: the first is a barrier removed and the third a supply of entrants, and both would move the numbers with no preference over composition running at all. The Threshold, The Mix. |

## Restrictions

What the house refuses to model, and how it credits what it stages.

- **No villains**: the elements are reduced to load-bearing vectors. The play refuses to cast the residents as bigots
  or the sort as a design, because the source finding is precisely that no bigotry and no plan is needed: each resident
  holds a mild, tolerant preference, wanting only not to be too small a minority, a wish a mixed street satisfies for
  everyone at once, and moves only when a threshold hidden inside that wish is crossed. No one prefers segregation, and
...
- **Credit, not claim**: the research above is other people's work, packaged and attributed here, never claimed as the
  ...
  Sakoda's, the general threshold-cascade mathematics is Granovetter's, and the empirical test that the driving
  preferences are mild is Clark's, each carried as carefully as the finding itself: the misfit stages tolerant wishes
  sorting a city and credits that the preferences are mild and lawful and the separation one no one chose.

## Encoding

Source to constraint, per element.

- **[The Preference](piece_the_preference.md)**: carries the mild, tolerant wish each resident holds (Schelling;
  Clark), the reluctance to be too small a minority that a mixed street satisfies for all at once.
- **[The Threshold](piece_the_threshold.md)**: carries the tip point hidden inside the preference (Grodzins;
  Granovetter), the like-share below which content turns to departure and a block begins to flip.
- **[The Mix](piece_the_mix.md)**: carries the local composition each resident reads and each move shifts (Sakoda;
  Schelling), the state that drifts smoothly until it crosses a threshold.
- **[The Tipping](piece_the_tipping.md)**: carries the phenomenon whole (Schelling; Granovetter), the cascade by which
  mild preferences compose, through a threshold, into a stark and sorted macro-segregation no one chose.
