...
[The Straight Rows](play_the_straight_rows.md). The seat of situated practice, where the working knowledge lives in the doing and is cleared by the grid as disorder.
...
Work by feel.
Run the system as the ground actually is, keeping the mixed stand and the rotation and the unplanned order, holding the place together by the thousand tacit adjustments no plan specifies.
Read the particular that the survey cannot, since the domain is carried by exactly what the map leaves out.
...
Toward a knowledge that carries the domain,
and a grid that cannot record it,
so the seat that actually runs the place is cleared as disorder by the map that means to run it.
