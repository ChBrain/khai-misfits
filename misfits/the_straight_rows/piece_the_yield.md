...
[The Straight Rows](play_the_straight_rows.md). The heavy first harvest, the early success that vindicates the legible method and spreads it, drawn on a reserve already cleared.
...
Comes in at [the forest](place_the_forest.md), the first rotation, counted at [the survey](place_the_survey.md) as the proof of the method.
Flows from the legible order to the ledger, the countable harvest that reads as the vindication of the grid.
...
Just proof the method works, the plan vindicated by a harvest anyone can count.
Seen at the surface, the Yield looks like the answer to every doubt, the legible order delivering exactly what it promised, and the truth that the first rotation drew on a reserve the clearing spent, that a success can be the last the ground affords, that the proof is drawn on borrowed soil, stays hidden behind how convincing a heavy harvest is.
