...
A domain made legible and made barren.
What leaves this process is the place the centre remade to read now failing on the map that reads it, the second rotation sick where the first was heavy, so the simplification that let the centre see the domain is the same act that hollowed it, a ruin with no tyrant, only a grid run faithfully over a knowledge it cleared to see.
