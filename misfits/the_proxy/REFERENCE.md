...
sign: positive # how far an individual is judged on their own qualities, as those qualities become cheaper to observe directly
...
Staging statistical discrimination as a **self-confirming-prior structure**: when an agent cannot cheaply see an
individual's quality, it prices the individual by the average of the group they belong to, the best cheap signal it
has, so the member of a group the agent rates low earns a lower return on any costly signal of quality, rationally
invests less in that signal because its return is discounted, and the group's real average falls to meet the low
rating, the assumed average becoming the produced average and the prior confirming itself, while the genuinely able
...
It is distinct from the house's The Market for Lemons, and the distinction is the whole point of the cut: the
lemons market pools unlike units of one good under a buyer who cannot tell them apart, so the pooling price drives
the good units out of trade and the market can unravel, whereas statistical discrimination pools unlike
individuals by the group they belong to, and the pooling does not drive the able out but prices them low and, worse,
makes the low price come true, the group's average bending to meet the rating in a loop the lemons market does not
have, a self-fulfilling prior rather than an adverse selection. It is also distinct from taste-based discrimination
in the manner of Becker, where an employer with a distaste for a group forgoes profit to avoid it: there the engine
is animus and the discriminator pays for it, here there is no distaste and no forgone profit, only an agent pricing
...
- **No villains**: the elements are reduced to load-bearing vectors. The play refuses to cast the employer as a
  bigot, because the source finding is precisely that no bigotry is needed: unable to see the individual's quality
  and pricing by the cheapest signal it has, the agent uses the group average because it is, on the numbers before
  it, the rational proxy, and the group member who invests less in a discounted signal is answering the return the
  market actually offers them. The discrimination is authored by no prejudice and emerges from a proxy met by a
  rational response, so the misfit is that a market of agents each pricing by the best cheap signal, and each group
...
- **[The Prior](piece_the_prior.md)**: carries the group average used as the proxy (Phelps), the belief the agent
